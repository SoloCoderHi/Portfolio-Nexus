...
Portfolio Nexus is a comprehensive portfolio management system built on an event-driven microservices architecture. It allows users to track their investments, analyze market data, and manage their financial portfolios effectively.
...
* **🤖 AI-Powered Market Data Parsing**: Utilizes an LLM (MistralAI) via LangChain to accurately extract market data from various sources.
...
* **🏛️ Microservices Architecture**: Decoupled services (Auth, User, Portfolio, Market Data) that communicate asynchronously using Kafka, ensuring high scalability and resilience.
* **📨 Event-Driven Communication**: Services react to events published on Kafka topics (e.g., `user_service`, `portfolioservice`), promoting loose coupling and independent service development.
...
```bash
git clone https://github.com/al0nec0der/Portfolio-Nexus.git
cd Portfolio-Nexus
````

### 2\. Configure Environment Variables

The `marketDataService` may require API keys for various data sources. Navigate to the `docker-compose.yml` file and replace the placeholder value.

In `docker-compose.yml`, find the `marketdataservice` definition and update the environment variables as needed:
...
The services will be available at the following ports:
  ...
  * **Portfolio Service**: `http://localhost:9811`
  * **Market Data Service**: `http://localhost:8010`
...
### 2\. Submit an SMS for Market Data Parsing

Use the `access_token` and `user_id` from the signup step to authenticate your request with the `marketDataService`.
...
**Response:**
The `marketDataService` will return the processed data and publish it to Kafka for the `portfolioService` to consume and update the user's portfolio.
...
Retrieve all portfolios for the user from the `portfolioService`.

```bash
curl --location 'http://localhost:9811/portfolio/v1/getPortfolio' \
--header "X-User-Id: $USER_ID" \
--header "Authorization: Bearer $ACCESS_TOKEN"
```
